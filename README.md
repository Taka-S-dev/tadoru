...
手元の Windows x86_64 / release ビルドで、既定の上限の 20 万件まで集めるフォルダを指定すると、
走査から並び順の確定まで 0.2 秒弱、`src` への検索更新は 10 ms 以下だった。実行ごとに動くので、
