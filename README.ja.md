...
- よく行く場所: `work` と名前を付けてお気に入りに登録し、どこにいても `c @work` でそこから探す
...
お気に入りと最近の場所は画面ではなく、今の画面の上に開く一覧。Ctrl-S と Ctrl-R、
またはタブの右の `favorites` `recent` のクリックで開き、Enter でそこへ cd、
Right で今の画面をそこへ移し、Esc で閉じる。
...
| Ctrl-S / Ctrl-R | お気に入り（S は ★ の starred）・最近の場所の一覧を画面の上に開く。タブの右の `favorites` `recent` のクリックでも開く。文字で絞り込み、Enter でそこへ cd、Right かダブルクリックで今の画面をそこへ移して続ける、Esc で閉じる |
